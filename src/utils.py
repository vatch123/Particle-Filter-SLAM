...
def mapCorrelation(im, x_im, y_im, vp, xs, ys):
  ...
  #     cpr[jx,jy] = np.clip(np.sum(im[ix[valid],iy[valid]]), a_min=1, a_max=None)
  # return cpr

  N = vp.shape[0]
  S = vp.shape[2]
  xs_mat = np.repeat(np.repeat(xs[None,None,:], N, axis=0), S, axis=1)
  ys_mat = np.repeat(np.repeat(ys[None,None,:], N, axis=0), S, axis=1)
  ...
  # valid = np.logical_and( np.logical_and((iy >=0), (iy < ny)), \
	# 		                        np.logical_and((ix >=0), (ix < nx)))
  matches = im[ix,iy].reshape((N,S,-1))
  corrs = np.clip(np.sum(matches, axis=1), a_min=1, a_max=None)
  return corrs
...
def test_mapCorrelation():
  ...
  ranges = np.load("./src/test_ranges.npy")
