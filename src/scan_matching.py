...
class ICP:
    ...
    @staticmethod
    def rotation_matrix(ang, direction="ccw"):
        d = 1
        if direction == "cw":
            d = -1
        elif direction == "ccw":
    ...
    def test_data(self, rotation=0., translation=np.zeros(2)):
        ...
        ds2_rot = np.array(ds2_rot) + translation
...
if __name__ == "__main__":
    from src.icp import icp as icp2
    from src.new_icp import icp as icp3
    icp = ICP()
    a, b = icp.test_data(.25, np.array([0.4, .75]))
    transformation, c, i = icp3(b, a, R_initial=np.eye(2), t_initial=np.zeros(2))
